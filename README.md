...
- **RGB色空間の3D理解**: RGB値を3次元ベクトルとして直感的に把握
...
本ツールは8つのタブで色彩理論を体系的に学べます（全60個のアコーディオン形式の解説）：

#### 1️⃣ タブ1: RGBカラースペース
- **RGBスライダー**: 赤・緑・青を0〜255で個別調整
- **3D RGBキューブ**: RGB色空間を3次元で可視化（ドラッグで回転可能）
- **キューブ内ポイント表示**: 現在の色がキューブ内のどこに位置するかを表示
- **プリセット＆コーナーボタン**: 原色・頂点の色をワンクリックで選択
- **色の類似度計算**: 2色間のコサイン類似度とユークリッド距離を算出
- **3D比較ビュー**: 2色のキューブ内位置と距離を視覚化
...
- **3D RGBキューブ**: ドラッグで回転、色の位置をリアルタイム表示（タブ1）
- **色の類似度計算**: 2色のベクトル比較をキューブ上で可視化（タブ1）
- **補色の色相環**: ホバーで補色ペアをハイライト表示（タブ2）
- **類似色相の色相環**: 隣接30度の類似色をインタラクティブに表示（タブ2）
- **トースト通知**: コピー成功時のフィードバック
- **レスポンシブデザイン**: モバイル（600px）・タブレット（768px）・デスクトップ完全対応

### 🛠️ 技術仕様
- **色変換アルゴリズム**: HSL⇔RGB⇔HEX双方向変換を実装
- **3D可視化**: CSS 3D transformによるRGBキューブのインタラクティブ表示
- **類似度計算**: コサイン類似度・ユークリッド距離による色比較機能
- **アコーディオンUI**: 60個の折りたたみ可能な解説セクション
